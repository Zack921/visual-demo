# 一些可视化的例子

- [canvas](#canvas)
  * [1.世界新冠疫情图](#1世界新冠疫情图)
- [webgl](#webgl)
  * [1.旋转三角形](#1旋转三角形)
  * [2.渐变三角形](#2渐变三角形)
  * [3.使用图像纹理](#3使用图像纹理)
  * [4.基础彩色立方体](#4基础彩色立方体)
  * [5.平行光下的光照模型(环境光 + 漫反射)](#5平行光下的光照模型环境光--漫反射)
  * [6.点光源下的光照模型(环境光 + 漫反射)](#6点光源下的光照模型环境光--漫反射)
  * [7.多关节模型(手臂)](#7多关节模型(手臂))
  * [8.基础交互-鼠标控制物体旋转](#8基础交互-鼠标控制物体旋转)
  * [9.基础交互-鼠标选中物体表面](#9基础交互-鼠标选中物体表面)
  * [10.利用阴影贴图实现阴影](#10利用阴影贴图实现阴影)
- [threejs](#threejs)
  * [1.github-contributions-3维图像](###[1github-contributions-3维图像])


## canvas
### 1.世界新冠疫情图
![world-covid](./assets/world-covid.gif)
[查看代码](./canvas2d/covid)

## webgl

### 1.旋转三角形
![rotatingTriangle](./assets/rotatingTriangle.gif)  
[查看代码](./webgl/examples/rotatingTriangle)

### 2.渐变三角形
![coloredTriangle](./assets/coloredTriangle.png)  
[查看代码](./webgl/examples/coloredTriangle)

### 3.使用图像纹理
![textureQuad](./assets/textureQuad.png)  
[查看代码](./webgl/examples/textureQuad)

### 4.基础彩色立方体
![coloredCube](./assets/coloredCube.png)  
[查看代码](./webgl/examples/coloredCube)

### 5.平行光下的光照模型(环境光 + 漫反射)
![parallelCube](./assets/parallelCube.gif)  
[查看代码](./webgl/examples/parallelCube)
### 6.点光源下的光照模型(环境光 + 漫反射)
![pointCube](./assets/pointCube.gif)  
[查看代码](./webgl/examples/pointCube)
### 7.多关节模型(手臂)
![multiJointModel](./assets/multiJointModel.gif)  
[查看代码](./webgl/examples/multiJointModel)
### 8.基础交互-鼠标控制物体旋转
![rotateByMouse](./assets/rotateByMouse.gif)  
[查看代码](./webgl/examples/rotateByMouse)
### 9.基础交互-鼠标选中物体表面
![pickFaceByMouse](./assets/pickFaceByMouse.gif)  
[查看代码](./webgl/examples/pickFaceByMouse)
### 10.利用阴影贴图实现阴影
![shadow](./assets/shadow.gif)  
[查看代码](./webgl/examples/shadow)
## threejs

### 1.github-contributions-3维图像
![github-contri](./assets/github-contri.gif)
[查看代码](./threejs/githubContribution)

